...
// Initialize records – always reset from defaults to ensure current state
...
function mostrarBandeja() {
  ...
  if (bandejaView) bandejaView.classList.remove('hidden');
  
  // Refresh records from localStorage to capture checklist status/comments updates
  records = JSON.parse(localStorage.getItem('efectiva_records')) || [...defaultRecords];
  
  // Re-apply filters and re-render table
  applyFilters();
}
...
function renderSolicitudesTable() {
  ...
  // All records appear as Pendiente in Solicitudes (no SLA, no Estado column)
  const solRecords = [...defaultRecords];

  if (solRecords.length === 0) {
    tbody.innerHTML = `<tr><td colspan="10" style="text-align: center; padding: 2rem; color: var(--text-muted); font-weight: 600;">No existen solicitudes pendientes.</td></tr>`;
    if (totalCountEl) totalCountEl.textContent = '0';
    return;
  }

  // Dynamic relative dates for display
  const now = new Date();
  const relativeSlaOffsets = {
    'EFE001': 1.2, 'EFE005': 4.5, 'EFE006': 2.1, 'EFE007': 5.2,
    'EFE004': 6.8, 'EFE003': 0.7, 'EFE009': 8.5, 'EFE002': 2.8, 'EFE008': 10.2
  };

  solRecords.forEach(r => {
    const offset = relativeSlaOffsets[r.solicitud] || 1;
    const displayDate = new Date(now.getTime() - (offset * 60 * 60 * 1000));
...
function confirmTomar() {
  ...
  closeTomarModal();
  showToast(`Solicitud ${tomarSolicitudId} tomada exitosamente.`);
  // Remove from solicitudes view (simulated)
  const row = document.querySelector(`#solicitudes-table-body tr`);
  // In a real app this would remove the specific row; for prototype just refresh
  renderSolicitudesTable();
}
